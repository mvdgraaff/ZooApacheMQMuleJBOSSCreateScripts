...
<xsl:variable name="tickTime" select="./zookeeper/tickTime"/>
# The number of milliseconds of each tick
tickTime=<xsl:value-of select="$tickTime"/>
# The number of ticks that the initial 
# synchronization phase can take
<xsl:variable name="initLimit" select="./zookeeper/initLimit"/>
initLimit=<xsl:value-of select="$initLimit"/>
# The number of ticks that can pass between 
# sending a request and getting an acknowledgement
<xsl:variable name="syncLimit" select="./zookeeper/syncLimit"/>
syncLimit=<xsl:value-of select="$tickTime"/>
# the directory where the snapshot is stored.
# do not use /tmp for storage, /tmp here is just 
# example sakes.
    
<xsl:variable name="dataDir" select="./zookeeper/dataDir"/>
dataDir=<xsl:value-of select="$dataDir"/>
# the port at which the clients will connect
<xsl:variable name="clientPort" select="./zookeeper/clientPort"/>
clientPort=<xsl:value-of select="$clientPort"/>
# the maximum number of client connections.
# increase this if you need to handle more clients
<xsl:variable name="maxClientCnxns" select="./zookeeper/maxClientCnxns"/>
...
<xsl:variable name="server.1" select="./zookeeper/server.1"/>
<xsl:variable name="server.2" select="./zookeeper/server.2"/>
<xsl:variable name="server.3" select="./zookeeper/server.3"/>
<xsl:variable name="server.4" select="./zookeeper/server.4"/>
